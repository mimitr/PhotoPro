...
def add_purchase(user_id, image_id, save_for_later, purchased, conn, cur):
    try:
        ...
        cmd = "INSERT INTO user_purchases (user_id, image_id, save_for_later, purchased)\
                    VALUES ({}, {}, {}, {})".format(int(user_id), int(image_id), save_for_later, purchased)
        cur.execute(cmd)
        conn.commit()
        return True
...
def delete_item_from_cart(user_id, image_id, conn, cur):
    try:
        ...
        cmd = "DELETE FROM user_purchases WHERE user_id={} AND image_id={}\
                AND purchased=FALSE".format(int(user_id), int(image_id))
        cur.execute(cmd)
        conn.commit()
        return True
    except psycopg2.Error as e:
...
def get_user_purchases(user_id, save_for_later, purchased, conn, cur):
    try:
        ...
        cmd = "SELECT user_id, images.image_id, save_for_later, purchased, title, caption,\
                ...
                WHERE user_id={} AND save_for_later={} AND purchased={}".format(int(user_id), save_for_later, purchased)
        cur.execute(cmd)
        conn.commit()
        result = cur.fetchall()
        return result
...
def update_user_purchases_details(user_id, image_id, save_for_later, purchased, conn, cur):
    try:
        cur.execute('SAVEPOINT save_point')
        cmd = "UPDATE user_purchases SET save_for_later={}, purchased={} \
                WHERE user_id={} AND image_id={}".format(save_for_later, purchased, int(user_id), int(image_id))
        cur.execute(cmd)
        conn.commit()
        return True
    except psycopg2.Error as e:
